package com.example.presentzadmin.adapter
...
import com.example.presentzadmin.databinding.ItemCategoryLayoutBinding
...
class CategoryAdapter(var context:Context,val list:ArrayList<CategoryModel>) :RecyclerView.Adapter<CategoryAdapter.CategoryViewHolder>(){
    ...
    override fun onBindViewHolder(holder: CategoryViewHolder, position: Int) {
        holder.binding.CategoryListTextView.text=list[position].cat
        Glide.with(context).load(list[position].img).into(holder.binding.CategoryListImage)
